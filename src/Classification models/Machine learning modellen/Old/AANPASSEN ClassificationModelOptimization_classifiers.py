"""
This file is used to compare various classifier within one eeg_channel.
...
#%% Set variables
channel_number = 4
classifiers = ['LR', 'DT', 'SVM', 'KNN', 'LDA', 'RF', 'XGBoost']
...
sampled_data_file_name = 'testFeatureData_1000_all_ages_above_6months.xlsx'
...
original_allFeaturedata = allFeatureData
...
for classifier in classifiers:
    ...
    print("Hyperparameter tuning %s completed: %s hours" % (classifier, (time.time() - start_time) / 3600))
    ...
    """
    if data == 'all':
        data_description = [include_age_categories, 'number of stages: %s, remove artifacts: %s'
                            % (number_of_stages, remove_artifacts)]
    if data == 'sampled':
        data_description = sampled_data_file_name

    dfModelResults = pd.Series({'Data': data_description,
                                'EEG Channel': eeg_channel,
                                'mrmr Feature selection': mrmr_selected_features,
                                'sfs Feature selection': feature_subset_sfs_names,
                                'Hyperparameters': optimal_hyperparameters,
                                'AUC CV score (mean)': np.mean(auc_cv_score),
                                'AUC CV score (std)': np.std(auc_cv_score),
                                'Accuracy CV score (mean)': np.mean(acc_cv_score),
                                'Accuracy CV score (std)': np.std(acc_cv_score),
                                'Cohens kappa CV score (mean)': np.mean(cohens_kappa_cv_score),
                                'Cohens kappa CV score (std)': np.std(cohens_kappa_cv_score)})
